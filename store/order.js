  ...
  getUserOrder({ commit }, payloads) {
    ...
    fireApp
      ...
      .then(() => {
        fireApp
          .database()
          .ref(`userOrders/${user.uid}/${payload}/`)
          .once("value")
  ...
  confirmPayment({ dispatch, commit }, payload) {
    ...
    fireApp
      ...
      .then(result => {
        ...
        return fireApp
          .storage()
          .ref(`payment/${productData.imageName}`)
          .put(image);
      })
      .then(snapshot => {
        snapshot.ref
          ...
          .then(() => {
            fireApp
              .database()
              .ref("orders/" + productData.orderId)
              .once("value")
              .then(function(snapshot) {
                ...
                fireApp
                  .database()
                  .ref("ordersed/" + productData.orderId)
                  .update(data);
              })
              .then(() => {
                return fireApp
              ...
              .then(() => {
                return fireApp
                  .database()
                  .ref(`ordersed/${productData.orderId}/detail`)
                  .update({ status: "confirm" });
              })
              .then(() => {
                return fireApp
                  .database()
                  .ref(`userOrders/${user.uid}/${productData.orderId}`)
                  .update({ status: "confirm" });
              })
              .then(() => {
                dispatch("getOrder");
